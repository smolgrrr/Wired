...
The v1 ingest accepts only fixed aggregate owner/operation/outcome buckets and optional 96-bit daily preview-correlation tokens. Event content, event IDs, pubkeys, relay URLs, secrets, and arbitrary labels fail validation. Envelopes are limited to 32 KiB, 100 aggregates, and 100 correlation observations.
...
- 1,000 distinct preview tokens per UTC day.
